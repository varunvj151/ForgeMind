 ...
 *
 * <p>Uses an in-memory H2 database via the {@code test} profile.
 * Redis is mocked so no external services are required.
 * Each test runs in a transaction that is rolled back on completion.
 *
 * <p>Note: Flyway is disabled in the test profile, so we seed the
 * {@code ROLE_USER} role manually in {@code @BeforeEach}.
 */
@SpringBootTest
...
class AuthControllerIntegrationTest {
    ...
    void register_duplicateUsername_returns409() throws Exception {
        ...
        RegisterRequest duplicate = new RegisterRequest(
                "Carol", "White", "carolwhite",
                "carol2@example.com", "password123"
        );
        mockMvc.perform(post("/api/v1/auth/register")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void register_shortPassword_returns400() throws Exception {
        RegisterRequest request = new RegisterRequest(
                "Frank", "Hill", "frankhill",
                "frank@example.com", "short"
        );
    ...
    void register_blankFirstName_returns400() throws Exception {
        RegisterRequest request = new RegisterRequest(
                "", "Hill", "frankhill2",
                "frank2@example.com", "password123"
        );
