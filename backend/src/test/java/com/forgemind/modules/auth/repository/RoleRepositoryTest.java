 ...
 *
 * <ul>
 *   <li>Auto-configures an H2 in-memory database
 *   <li>Runs each test in a transaction that is rolled back on completion
 *   <li>Only loads JPA-related beans — no web or security context
 * </ul>
 */
@DataJpaTest
@ActiveProfiles("test")
...
class RoleRepositoryTest {
  ...
  @BeforeEach
  void setUp() {
    Role role = Role.builder().name("ROLE_USER").description("Standard authenticated user").build();
  ...
  void save_newRole_persistsWithId() {
    Role adminRole =
        Role.builder().name("ROLE_ADMIN").description("Platform administrator").build();
