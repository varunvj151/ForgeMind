 ...
 *
 * <p>Uses H2 in-memory DB via the {@code test} profile. Redis is mocked.
 * A real user is registered before each test using the registration endpoint
 * so that login can be verified end-to-end against actual BCrypt hashes.
 *
...
class AuthControllerLoginIntegrationTest {
    ...
    void login_errorMessageIsOpaque() throws Exception {
        // Wrong password
        MvcResult wrongPass = mockMvc.perform(post(LOGIN_URL)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        // Unknown user
        MvcResult unknownUser = mockMvc.perform(post(LOGIN_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new LoginRequest("ghost", "password123"))))
                .andReturn();

        String msgWrongPass  = objectMapper.readTree(wrongPass.getResponse().getContentAsString())
                .at("/error/message").asText();
        String msgUnknownUser = objectMapper.readTree(unknownUser.getResponse().getContentAsString())
                .at("/error/message").asText();
