 ...
 *
 * <p>This class is the only place that contains business logic for the Project module. It never
 * exposes JPA entities outside its boundary — all return values are DTOs.
 */
@Slf4j
@Service
...
public class ProjectServiceImpl implements ProjectService {
  ...
  /**
   * Fetches a Project by ID or throws a {@link ResourceNotFoundException}. Centralises the
   * not-found logic so it is never duplicated across methods.
   */
        ...
        .orElseThrow(
            () -> {
              ...
              return new ResourceNotFoundException(
                  "PROJECT_NOT_FOUND", "Project not found with id: " + id);
            });
  }

  /**
   * Checks if the current authenticated user is the owner of the project. Throws an
   * AccessDeniedException if they are not.
   */
  private void authorizeOwner(Project project) {
    User currentUser = currentUserProvider.getCurrentUser();
    if (!project.getOwner().getId().equals(currentUser.getId())) {
      log.warn(
          "Access denied: User {} attempted to modify Project {} owned by User {}",
          currentUser.getId(),
          project.getId(),
