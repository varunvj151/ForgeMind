 ...
 *
 * <p>Algorithm: HMAC-SHA256 (HS256) — symmetric, single-secret scheme. The secret is loaded from
 * {@code app.jwt.secret} via {@link JwtProperties}.
 *
 * <p>Token structure:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The subject ({@code sub}) claim stores the username, which is used by {@link
 * JwtAuthenticationFilter} to reload the {@link UserDetails} from the database.
 */
@Slf4j
@Service
...
public class JwtService {
   ...
   *
   * <p>The refresh token uses the same HS256 signing key as the access token but carries a much
   * longer expiry driven by {@code app.jwt.refresh-expiration-ms}. It is stored (as a SHA-256 hash
   * of the opaque random value) in the {@code refresh_tokens} table and used only to issue new
   * access tokens.
   *
   * @param userDetails the authenticated user
   ...
   *
   * <ol>
   *   <li>Its signature matches (verified internally by JJWT)
   *   <li>It has not expired
   *   <li>The {@code sub} claim matches the username in {@code userDetails}
   * </ol>
   *
  ...
  /**
   * Derives the {@link SecretKey} from the base64-encoded secret string in properties. The key is
   * used for both signing new tokens and verifying incoming ones.
   */
