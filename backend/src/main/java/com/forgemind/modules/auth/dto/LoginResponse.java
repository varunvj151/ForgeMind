 ...
 *
 * <p>Contains both JWT tokens and the authenticated user's public profile.
 * Passwords and password hashes are never included.
 *
 * <p>The {@code refreshToken} is an opaque random value (UUID). The server
 * stores only its SHA-256 hash, so the raw token is unrecoverable from the
 * database even if it is compromised.
 *
...
public record LoginResponse(
        ...
        @Schema(description = "Short-lived JWT access token (HS256).", example = "eyJhbGciOiJIUzI1NiJ9...")
        String accessToken,
