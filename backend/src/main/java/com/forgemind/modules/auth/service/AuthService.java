...
public class AuthService {
  ...
  public UserResponse registerUser(RegisterRequest request) {
    ...
    if (userRepository.existsByUsername(username)) {
      throw new ForgemindException(
          "USERNAME_EXISTS", "Username is already taken", HttpStatus.CONFLICT);
    }

    if (userRepository.existsByEmail(email)) {
      throw new ForgemindException("EMAIL_EXISTS", "Email is already taken", HttpStatus.CONFLICT);
    }
    ...
    log.info(
        "Successfully registered user: {} with ID: {}", savedUser.getUsername(), savedUser.getId());
   ...
   *
   * <ol>
   *   <li>Resolve the {@link User} by username or email — throws {@code 401} if not found.
   *   <li>Delegate credential verification to {@link AuthenticationManager} (BCrypt check + account
   *       status checks handled by Spring Security automatically).
   *   <li>On success: generate access token and refresh token via {@link JwtService}.
   *   <li>Delete all expired refresh tokens for the user, then persist the new one.
   * </ol>
   *
   * <p>On any authentication failure the error code is always {@code INVALID_CREDENTIALS} — the
   * response never reveals whether the identifier or the password was wrong.
   *
  ...
  public LoginResponse loginUser(LoginRequest request) {
    ...
    try {
      ...
      log.debug(
          "Authentication failure for identifier '{}': {}",
          identifier,
    ...
    return new LoginResponse(
        accessToken, refreshToken, "Bearer", expiresInSeconds, UserResponse.fromUser(user));
  }

  // ── Private Helpers ───────────────────────────────────────────────────────

  /**
   * Returns the SHA-256 hex digest of the given token string. Only the hash is stored in the
   * database, so raw tokens cannot be recovered even if the table is compromised.
   *
