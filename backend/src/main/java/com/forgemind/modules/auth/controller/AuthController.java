 ...
 *
 * <p>All endpoints under {@code /api/v1/auth/**} are publicly accessible (no JWT token required) as
 * configured in {@link com.forgemind.config.SecurityConfig}.
 */
@RestController
...
public class AuthController {
   ...
   *
   * <p>The password is hashed with BCrypt before storage. The plaintext password is never logged or
   * stored. On success, the new user is assigned {@code ROLE_USER}.
   *
  ...
  @Operation(
      ...
      description =
          "Creates a new user account with ROLE_USER. Password is hashed with BCrypt. "
   ...
   *
   * <p>Accepts either a registered username or an email address in the {@code usernameOrEmail}
   * field. On success returns a short-lived access token (default 15 min / {@code
   * app.jwt.expiration-ms}) and a long-lived refresh token (default 7 days / {@code
   * app.jwt.refresh-expiration-ms}).
   *
   * <p>Authentication failures always return {@code 401} with code {@code INVALID_CREDENTIALS} —
