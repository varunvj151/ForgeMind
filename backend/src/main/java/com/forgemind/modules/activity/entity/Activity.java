 ...
 *
 * <p>Activities are NEVER updated after creation. All columns are marked {@code updatable = false}.
 * There is no {@code @PreUpdate} hook.
 *
 * <p>Foreign-key references to project/team/task are stored as bare UUIDs (not @ManyToOne) so that
 * activities survive entity deletion and serve as a durable audit trail.
 */
@Entity
...
public class Activity {
  ...
  /**
   * Structured metadata stored as a JSON string. Contains context-specific key/value pairs (e.g.,
   * old/new status, assignee name).
   */
