...
public class ActivityServiceImpl implements ActivityService {
  ...
  /**
   * Records an activity within the CURRENT transaction. Uses {@link Propagation#REQUIRED} so it
   * participates in the caller's transaction — if the main operation rolls back, the activity is
   * also rolled back, maintaining consistency.
   */
  @Override
  ...
  public void record(
    ...
    log.debug(
        "Activity recorded: type={}, actor={}, project={}, team={}, task={}",
        type,
        actor.getId(),
        projectId,
        teamId,
        ...
        .ifPresent(
            project -> {
              if (!project.getOwner().getId().equals(currentUser.getId())) {
                log.warn(
                    "Unauthorized project activity access: user={}, project={}",
                    currentUser.getId(),
                    projectId);
                throw new AccessDeniedException(
                    "You do not have permission to view this project's activity feed.");
              }
            });
  ...
  public Page<ActivityResponse> getTeamActivities(UUID teamId, Pageable pageable) {
    ...
    if (!teamMemberRepository.existsByTeamIdAndUserId(teamId, currentUser.getId())) {
      ...
      throw new AccessDeniedException(
          "You must be a member of this team to view its activity feed.");
    }

    return activityRepository
        .findByTeamIdOrderByCreatedAtDesc(teamId, pageable)
        .map(activityMapper::toResponse);
  }
