...
public class TeamServiceImpl implements TeamService {
    ...
    public TeamResponse createTeam(TeamRequest request) {
        ...
        log.info("Team created: id={}, name={} by user={}", savedTeam.getId(), savedTeam.getName(), currentUser.getId());
        activityService.record(currentUser, ActivityType.TEAM_CREATED, "Team created", null, savedTeam.getId(), null, null);
    ...
    public Page<TeamResponse> listTeams(Pageable pageable) {
        User currentUser = currentUserProvider.getCurrentUser();
        return teamMemberRepository.findAllByUserId(currentUser.getId(), pageable)
    ...
    public TeamMemberResponse addMember(UUID teamId, AddTeamMemberRequest request) {
        ...
        if (teamMemberRepository.existsByTeamIdAndUserId(teamId, request.userId())) {
            throw new ForgemindException("DUPLICATE_MEMBER", "User is already a member of this team", HttpStatus.CONFLICT);
        }
        ...
        activityService.record(currentUserProvider.getCurrentUser(), ActivityType.TEAM_MEMBER_ADDED, "User added to team", null, teamId, null, java.util.Map.of("targetUserId", targetUser.getId(), "role", request.role().name()));
        return teamMapper.toResponse(saved);
    }
    ...
    public void removeMember(UUID teamId, Long userId) {
        ...
        TeamMember memberToRemove = teamMemberRepository.findByTeamIdAndUserId(teamId, userId)
                .orElseThrow(() -> new ResourceNotFoundException("MEMBER_NOT_FOUND", "User is not a member of this team"));

        if (memberToRemove.getRole() == TeamRole.OWNER) {
            throw new ForgemindException("CANNOT_REMOVE_OWNER", "Cannot remove an owner from the team. Transfer ownership first.", HttpStatus.BAD_REQUEST);
        }

        teamMemberRepository.delete(memberToRemove);
        log.info("Removed user={} from team={}", userId, teamId);
        activityService.record(currentUserProvider.getCurrentUser(), ActivityType.TEAM_MEMBER_REMOVED, "User removed from team", null, teamId, null, java.util.Map.of("targetUserId", userId));
    }
    ...
    private void authorizeRole(UUID teamId, TeamRole... allowedRoles) {
        ...
        TeamMember currentMember = teamMemberRepository.findByTeamIdAndUserId(teamId, currentUser.getId())
