...
public class TaskController {
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "403",
        description = "Not the owner or assignee",
        content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    @ApiResponse(
        responseCode = "404",
  ...
  public ResponseEntity<TaskResponse> changePriority(
      @PathVariable UUID taskId, @Valid @RequestBody UpdateTaskPriorityRequest request) {
    return ResponseEntity.ok(taskService.changePriority(taskId, request));
  }

  @Operation(
      summary = "Assign or un-assign a task (owner only). Pass null assigneeId to un-assign.")
  @ApiResponses({
  ...
  public ResponseEntity<TaskResponse> assignTask(
      @PathVariable UUID taskId, @Valid @RequestBody UpdateTaskAssigneeRequest request) {
