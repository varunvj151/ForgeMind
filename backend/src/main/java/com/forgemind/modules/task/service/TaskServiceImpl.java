...
public class TaskServiceImpl implements TaskService {
    ...
    public TaskResponse createTask(UUID projectId, CreateTaskRequest request) {
        ...
        log.info("Task created: id={}, title={}, project={}, by={}", 
                saved.getId(), saved.getTitle(), projectId, currentUser.getId());
                
        activityService.record(currentUser, ActivityType.TASK_CREATED, "Task created", projectId, null, saved.getId(), null);
    ...
    public TaskResponse updateTask(UUID taskId, UpdateTaskRequest request) {
        ...
        activityService.record(currentUser, ActivityType.TASK_UPDATED, "Task updated", task.getProject().getId(), null, saved.getId(), null);
        return taskMapper.toResponse(saved);
    }
    ...
    public void deleteTask(UUID taskId) {
        ...
        activityService.record(currentUser, ActivityType.TASK_DELETED, "Task deleted", task.getProject().getId(), null, taskId, null);
    }
    ...
    public TaskResponse changeStatus(UUID taskId, UpdateTaskStatusRequest request) {
        ...
        if (!isOwner && !isAssignee) {
            ...
            throw new AccessDeniedException("Only the project owner or task assignee may update the task status.");
        }
        ...
        if (request.status().name().equals("DONE")) {
            ...
            activityService.record(currentUser, ActivityType.TASK_COMPLETED, "Task completed", task.getProject().getId(), null, saved.getId(), null);
        } else {
            log.info("Task status changed: id={}, status={}, by={}", taskId, request.status(), currentUser.getId());
            activityService.record(currentUser, ActivityType.TASK_STATUS_CHANGED, "Task status changed", task.getProject().getId(), null, saved.getId(), java.util.Map.of("status", request.status().name()));
        }
        return taskMapper.toResponse(saved);
    }
    ...
    public TaskResponse changePriority(UUID taskId, UpdateTaskPriorityRequest request) {
        ...
        log.info("Task priority changed: id={}, priority={}, by={}", taskId, request.priority(), currentUser.getId());
        activityService.record(currentUser, ActivityType.TASK_UPDATED, "Task priority changed", task.getProject().getId(), null, saved.getId(), java.util.Map.of("priority", request.priority().name()));
        return taskMapper.toResponse(saved);
    }
    ...
    public TaskResponse assignTask(UUID taskId, UpdateTaskAssigneeRequest request) {
        ...
        if (request.assigneeId() == null) {
            ...
            User assignee = findUserOrThrow(request.assigneeId());
            task.setAssignee(assignee);
            log.info("Task assigned: id={}, assignee={}, by={}", taskId, assignee.getId(), currentUser.getId());
        }

        Task saved = taskRepository.save(task);
        activityService.record(currentUser, ActivityType.TASK_ASSIGNED, "Task assignment updated", task.getProject().getId(), null, saved.getId(), java.util.Map.of("assigneeId", request.assigneeId() == null ? "unassigned" : request.assigneeId()));
        return taskMapper.toResponse(saved);
    }
    ...
    public Page<TaskResponse> listMyTasks(Pageable pageable) {
        User currentUser = currentUserProvider.getCurrentUser();
        return taskRepository.findByAssigneeId(currentUser.getId(), pageable)
                .map(taskMapper::toResponse);
    }
    ...
    private Task findTaskOrThrow(UUID taskId) {
        return taskRepository.findById(taskId)
                .orElseThrow(() -> {
                    ...
                    return new ResourceNotFoundException("TASK_NOT_FOUND", "Task not found with id: " + taskId);
                });
    }
    ...
    /**
     * Verifies the current user is the project owner.
     * All destructive operations (create, update, delete, assign, priority) require this.
     */
    private void authorizeProjectOwner(Project project, User currentUser) {
        if (!project.getOwner().getId().equals(currentUser.getId())) {
            log.warn("Unauthorized access: user={} attempted action on project={} owned by={}",
                    currentUser.getId(), project.getId(), project.getOwner().getId());
            throw new AccessDeniedException("You do not have permission to perform this action on this project.");
        }
    }
